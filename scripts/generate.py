  ...
  <style>
    *, *::before, *::after {{ box-sizing: border-box; margin: 0; padding: 0; }}
    ...
    .row-top-2 {{
      ...
      background-size: 250% 100%;
      animation: shimmer-pass 4s ease-in-out 0.8s infinite;
    }}
    .row-top-3 {{
      background-image: linear-gradient(100deg, transparent 20%, rgba(184,115,51,0.09) 50%, transparent 80%);
      background-size: 250% 100%;
      animation: shimmer-pass 3.5s ease-in-out 1.6s infinite;
    }}
  ...
  <script>
    (function () {{
      ...
      glow.style.cssText = [
        ...
        'background:radial-gradient(circle,rgba(47,129,247,0.07) 0%,transparent 68%)',
      ...
      function loop() {{
        ...
        if (dark) {{
          for (var j = 0; j < pts.length; j++) {{
            ...
            pc.fillStyle = 'rgba(255,255,255,' + p.o + ')';
            pc.fill();
            p.x += p.vx; p.y += p.vy;
